...
- **文件名规范化** — 自动将分解形式（NFD）的文件名重命名为 NFC，修复韩文、
  日文等语言中看起来乱码的文件名。
- **窗口切换器** — Windows 风格的 Cmd+Tab：按单个窗口而非 App 进行切换。
  缩略图网格可用 Tab、方向键或鼠标操作；授予屏幕录制权限后显示窗口预览
  （未授权时显示 App 图标）。
...
程序坞锚定、滚动方向、截图自动拷贝和窗口切换器这几个模块需要辅助功能权限。
窗口切换器另可选择授予屏幕录制权限，以显示窗口预览。请在
**系统设置 > 隐私与安全性 > 辅助功能** 中授予权限——需要时 App 会引导你前往。
其余模块无需任何特殊权限即可工作。

> **注意:** 当其他 App 持有 macOS **安全键盘输入(Secure Keyboard Input)**
> 时(例如 Terminal 正在显示密码提示或全屏终端界面),macOS 会在系统层面阻止
> 所有 App 的键盘拦截,截图自动拷贝会暂时回退到原生截图行为。macssential
> 检测到该状态后会显示警告并指出持有锁定的 App,锁定解除后自动恢复。
