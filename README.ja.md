...
- **ウィンドウスイッチャー** — WindowsスタイルのCmd+Tab。アプリ単位ではなく
  個別のウィンドウ単位で切り替えます。サムネイルグリッドをTab・矢印キー・
  マウスで操作でき、画面収録の許可があればウィンドウのプレビューが表示され
  ます（未許可の場合はアプリアイコン）。
...
Dock固定、スクロール方向、スクリーンショット自動コピー、ウィンドウ
スイッチャーの各モジュールにはアクセシビリティ権限が必要です。ウィンドウ
スイッチャーは、画面収録権限（任意）を許可するとウィンドウのプレビューを
表示します。**システム設定 > プライバシーとセキュリティ >
アクセシビリティ** で許可してください — 必要になったタイミングでアプリが案内
します。それ以外のモジュールは特別な権限なしで動作します。

> **注:** 他のアプリが macOS の**セキュリティキーボード入力(Secure Keyboard
> Input)** を保持している間(例: Terminal のパスワード入力や全画面のターミナル
> UI)は、macOS がシステム全体のキーボード横取りをブロックするため、スクリーン
> ショット自動コピーは一時的にネイティブのスクリーンショット動作になります。
> macssential はこの状態を検出すると、ロックを保持しているアプリ名を警告として
> 表示し、解除されると自動的に復帰します。
