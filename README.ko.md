...
- **윈도우 전환기** — Windows 스타일의 Cmd+Tab: 앱 단위가 아니라 개별 창
  단위로 전환합니다. 썸네일 그리드를 Tab·방향키·마우스로 조작할 수 있고,
  화면 기록 권한을 허용하면 창 미리보기가 표시됩니다(미허용 시 앱 아이콘).
...
Dock 고정, 스크롤 방향, 스크린샷 자동 복사, 윈도우 전환기 모듈은 손쉬운
사용 권한이 필요합니다. 윈도우 전환기는 추가로 화면 기록 권한(선택)을
허용하면 창 미리보기를 표시합니다. **시스템 설정 > 개인정보 보호 및 보안 > 손쉬운 사용**에서 권한을
허용해 주세요 — 필요할 때 앱이 해당 위치로 안내합니다. 나머지 모듈은 별도의
권한 없이 동작합니다.

> **참고:** 다른 앱이 macOS **보안 키보드 입력(Secure Keyboard Input)** 을
> 잡고 있는 동안(예: Terminal의 비밀번호 프롬프트나 전체 화면 터미널 UI)에는
> macOS가 시스템 전체의 키보드 가로채기를 차단하므로, 스크린샷 자동 복사가
> 일시적으로 기본 스크린샷 동작으로 대체됩니다. macssential이 이 상태를
> 감지하면 어떤 앱이 잠금을 잡고 있는지 경고로 표시하고, 해제되면 자동으로
> 복구됩니다.
